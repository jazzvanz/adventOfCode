// This is a very bad answer but your girls rusty & tired. 
...
// Convert to Array of Intergers/Numbers
const expenseArray = expenseReport.split(' ').map(function (expenseItem) {
  return parseInt(expenseItem, 10)
})

let i = 0

const reportAmounts = expenseArray.length

while (i < reportAmounts) {
  const val = expenseArray[i]
  for (let two = 0; two < reportAmounts; two++) {
    const secondValue = expenseArray[two]
    for (let three = 0; three < reportAmounts; three++) {
      const thirdValue = expenseArray[three]
      const total = val + secondValue + thirdValue
      if (total === 2020) {
        const multiAss = val * secondValue * thirdValue
        console.log(total, 'MATCH', multiAss)
        break
      }
    }
  }
